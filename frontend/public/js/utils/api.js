...
function rewriteEndpoint(endpoint, options) {
    var method = (options.method || 'GET').toUpperCase();
    var q = '?';
    var eq = '=';
    var amp = '&';

    // /auth?action=X patterns
    if (endpoint.startsWith('/auth?action=')) {
        var action = endpoint.split(eq)[1].split(amp)[0];
        var map = {
            'email_login':       { path: '/auth/email-login',      method: 'POST' },
            'register':          { path: '/auth/register',         method: 'POST' },
            'session':           { path: '/auth/session',          method: 'GET' },
            'logout':            { path: '/auth/logout',           method: 'POST' },
            'login':             { path: '/auth/google/url',       method: 'GET' },
            'forgot_password':   { path: '/auth/forgot-password',  method: 'POST' },
            'reset_password':    { path: '/auth/reset-password',   method: 'POST' },
            'dev_login':         { path: '/auth/dev-login',        method: 'POST' },
            'update_profile':    { path: '/auth/profile',          method: 'PUT' },
            'upload_avatar':     { path: '/auth/avatar',           method: 'POST' },
        };
        var rule = map[action];
        if (rule) {
            options.method = rule.method;
            return rule.path;
        }
    }

    // /auth?action=callback&code=...
    if (endpoint.startsWith('/auth?action=callback')) {
        var code = endpoint.split('code=')[1] || '';
        return '/auth/google/callback?code=' + code;
    }

    // /auth?action=X (with apiRequest default method)
    if (endpoint.startsWith('/auth?action=')) {
        return '/auth/action';
    }

    // /events?id=X, /users?id=X, /alerts?id=X, /distress?id=X
    m = endpoint.match(/^\/(events|users|alerts|distress)\?id=(\d+)$/);
    if (m) {
        return '/' + m[1] + '/' + m[2];
    }

    // /system?action=X patterns
    if (endpoint.startsWith('/system?action=')) {
        var act = endpoint.split(eq)[1].split(amp)[0];
        var idx = endpoint.indexOf(amp);
        var qs = idx !== -1 ? '?' + endpoint.substring(idx + 1) : '';
        var sysMap = {
            'health':           '/system/health',
            'report':           '/system/report',
            'archive':          '/system/archive',
            'escalate':         '/system/escalate',
            'logs':             '/system/logs',
            'export_csv':       '/system/export-csv',
            'export-xml':       '/system/export-xml',
            'import-xml':       '/system/import-xml',
            'retry-dead-letter':'/system/retry-dead-letter',
            'resources':        '/system/resources',
            'timeline':         '/system/timeline',
            'add-resource':     '/system/add-resource',
        };
        var sp = sysMap[act];
        if (sp) {
            var qp = '';
            if (act === 'archive' || act === 'escalate') { options.method = 'POST'; }
            if (act === 'import-xml' || act === 'retry-dead-letter') { options.method = 'POST'; }
            if (qs) qp = qs;
            return sp + qp;
        }
    }

    // /announcements (no such route — leave as-is so it 404s visibly)
    return endpoint;
}

async function apiRequest(endpoint, options) {
    if (!options) options = {};
    endpoint = rewriteEndpoint(endpoint, options);
...
function connectWebSocket() {
    if (!WS_URL || (wsConnection && wsConnection.readyState === WebSocket.OPEN)) return;
    try {
...
async function handleEmailLogin() {
    ...
    try {
        var data = await apiRequest('/auth?action=email_login', {
...
async function handleRegister() {
    ...
    try {
        var data = await apiRequest('/auth?action=register', {
            method: 'POST', body: JSON.stringify({ display_name: displayName, email: email, password: password, role: role }),
        });
        if (data.success) {
            authToken = data.token; currentUser = data.user;
            localStorage.setItem('ems_token', data.token); localStorage.setItem('ems_user', JSON.stringify(data.user));
...
async function handleDevLogin() {
    ...
    try {
        var response = await fetch(API_BASE + '/auth/dev-login', {
...
function startMessagePolling() {
    ...
    messagePollInterval = setInterval(function() {
        var eventId = document.getElementById('message-event-select');
...
async function loadDistressSignals() {
    ...
    try {
        ...
        console.error('Distress load error:', err);
...
function toggleSidebar() {
    ...
    setTimeout(function() {
        if (typeof updateChatHeight === 'function') updateChatHeight();
    }, 300);
}
...
async function exportCSV(type) {
    try {
        var resp = await fetch(API_BASE + '/system/export-csv?type=' + encodeURIComponent(type), {
            headers: { 'Authorization': 'Bearer ' + authToken },
        });
        if (!resp.ok) { showToast('Export failed', 'error'); return; }
        var blob = await resp.blob();
        var url = URL.createObjectURL(blob);
        var a = document.createElement('a'); a.href = url; a.download = type + '.csv'; a.click();
...
function printReport() {
    if (currentUser && currentUser.role === 'victim') { showToast('Access denied', 'error'); return; }
    apiRequest('/events').then(function(data) {
        var events = data.events || [];
...
async function saveProfile() {
    ...
    try {
        ...
        if (avatarFile) {
            ...
            var uploadResp = await fetch(API_BASE + '/auth/avatar', {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + authToken },
                body: formData,
            });
...
function showPage(pageId) {
    ...
    switch (pageId) {
        ...
        case 'messages': populateEventSelectors(); startMessagePolling(); break;
